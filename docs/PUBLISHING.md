...
Снимки окна расширения — `popup.png` и `screenshot-5-popup.png` — показывают
номер версии: он берётся из манифеста. Значит переснимать их имеет смысл
**после** того, как релизный пул-реквест слит и номер поднят, иначе в магазин
уедет картинка с прошлой версией.
