...
(function (global) {
   ...
   *
   * FNV-1a по байтам строки: строки сравниваются только на равенство, а
   * хранить ради этого сами байты — лишние мегабайты на каждую картинку.
   * Совпадение хешей у разных строк возможно, но цена ошибки здесь мала:
   * неверно сшитая пара строк тут же разойдётся попиксельным сравнением.
   */
  function rowHashes(data, width, height) {
    ...
    for (let y = 0; y < height; y++) {
      let hash = 0x811c9dc5;
      const start = y * bytes;
      for (let i = start; i < start + bytes; i++) {
        hash ^= data[i];
