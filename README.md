...
**Safari, macOS** — `npm run build:safari` (нужен Xcode). Собрать полученный
проект, затем в Safari: Разработка → Разрешить неподписанные расширения, и
включить расширение в настройках.

**Safari, iPhone и iPad** — тот же проект, схема с суффиксом `(iOS)`, сборка на
своё устройство из Xcode. Дальше Настройки → Приложения → Safari → Расширения.
...
| Safari, macOS и iOS | Сборка не проверена: конвертеру Apple нужен донастроенный Xcode (`sudo xcodebuild -runFirstLaunch`) |
